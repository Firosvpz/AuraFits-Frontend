...
const Users = () => {
  ...
  const calculateAnalytics = (usersData) => {
    ...
    const recentUsers = usersData.filter(
      (u) => new Date(u.joinedAt) >= last7Days,
    ).length;
    ...
    const previousUsers = usersData.filter(
      (u) =>
        new Date(u.joinedAt) >= previous7Days &&
        new Date(u.joinedAt) < last7Days,
    ).length;
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-gray-400 mt-1">
                Manage {analytics.totalUsers} users with advanced analytics and
                filtering
              </p>
            </div>
          </div>
